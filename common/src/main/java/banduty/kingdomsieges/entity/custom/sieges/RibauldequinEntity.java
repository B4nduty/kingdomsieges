...
public class RibauldequinEntity extends AbstractSiegeEntity implements GeoEntity {
    ...
    private static final SiegeProperties PROPERTIES = SiegeProperties.builder("ribauldequin")
            ...
            .moveSound(KSSounds.SIEGE_ENGINE_MOVE.get())
            .shootSound(KSSounds.CANNON_CLOSE.get())
            .moveSoundDelay(150)
            .moveSoundRange(30.0)
    ...
    private static final LoadingStage[] LOAD_STAGES = {
            // Barrel 1
            LoadingStage.of(SCItems.BLACK_POWDER.get(), 2),
            LoadingStage.ofDamaging(KSItems.RAMROD.get()),
    ...
    private void fireSingleShot(ServerLevel serverLevel, float angleOffset) {
        CannonProjectile projectile = new CannonProjectile(this, serverLevel);
        ...
        spawnParticleTrail(serverLevel, direction.normalize(), mouthPos, SCParticles.MUZZLES_SMOKE_PARTICLE.get(), 20, 0.2f, 0.0005f, 5);
        spawnParticleTrail(serverLevel, direction.normalize(), mouthPos, SCParticles.MUZZLES_FLASH_PARTICLE.get(), 1, 0f, 0.1f, 6);
    }
