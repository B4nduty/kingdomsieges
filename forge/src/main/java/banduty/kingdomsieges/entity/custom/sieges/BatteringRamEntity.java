        ...
        if (this.getCooldown() <= 0 && player.isShiftKeyDown()) {
            ...
            serverLevel.players().forEach(p -> {
                ...
                if (distance <= 15) {
                    ...
                    if (volume > 0f) {
                        p.playNotifySound(ModSounds.ROPE_CHARGE_BR.get(), SoundSource.AMBIENT, volume, random.nextFloat(0.75f, 1.25f));
                    }
                }
            });
        }
        ...
        if (this.getCooldown() <= 6 && !this.getAttackHappened()) {
            ...
            serverLevel.players().forEach(p -> {
                ...
                if (distance <= 40) {
                    ...
                    if (volume > 0f) {
                        p.playNotifySound(ModSounds.RAM_IMPACT.get(), SoundSource.AMBIENT, volume, random.nextFloat(0.75f, 1.25f));
                    }
                }
            });
        }
